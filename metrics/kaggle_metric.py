...
def find_best_match(
    ...
    Args:
        gts: (List[List[Union[int, float]]])
            Coordinates of the available ground-truth boxes
        ...
        ious: (np.ndarray)
            len(gts) x len(preds) matrix for storing calculated ious.
...
def calculate_precision(
    gts, preds, threshold=0.5, form="coco", ious=None
) -> float:
...
def calculate_image_precision(
    gts, preds, thresholds=(0.5,), form="coco"
) -> float:
